...
export default function TableList() {
  ...
  useEffect(() => {
    api.get('/employees').then((response) => {
      setEmpolyes(response.data);
    });
  }, []);

  const funcionarios = empolyees.map((obj) => {
    const { idfuncionario, type, name, firm, role } = obj;
    return [idfuncionario.toString(), name, role, firm, type];
  });
        ...
        <Card>
          ...
          <CardBody>
            <Table
              ...
              tableHead={['Id', 'Nome', 'Função', 'Empresa', 'Tipo']}
